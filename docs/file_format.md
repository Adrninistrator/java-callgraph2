...
## 1.40. MyBatis XML中格式化后的sql文本（使用MySQL）
...
## 1.41. 使用MyBatis时get/set方法所关联的数据库信息（使用MySQL）
...
## 1.42. MyBatis的XML中select的字段信息（使用MySQL）
...
## 1.43. MyBatis的XML中update set子句的字段信息（使用MySQL）
...
## 1.44. MyBatis Mapper方法操作的数据库表信息（使用MySQL）
...
## 1.45. MyBatis的XML中where子句的字段信息（使用MySQL）
...
## 1.46. MyBatis Mapper方法写的数据库表信息（使用MySQL）
...
## 1.50. static、final字段初始化方法信息（含枚举）
