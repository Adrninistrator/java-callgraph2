...
## 1.32. MyBatis的Entity与数据库字段名信息（使用MySQL）
...
## 1.33. MyBatis的Entity与Mapper、表名（使用MySQL）
...
## 1.34. 使用MyBatis时get/set方法所关联的数据库信息（使用MySQL）
...
## 1.35. MyBatis的XML中select的字段信息（使用MySQL）
...
## 1.36. MyBatis的XML中update set子句的字段信息（使用MySQL）
...
## 1.38. MyBatis的XML中where子句的字段信息（使用MySQL）
...
## 1.39. MyBatis Mapper方法写的数据库表信息（使用MySQL）
...
|6|注解类名|
|7|唯一类名|
|8|方法可能用于文件上传，1:是，0:否|
|9|方法可能用于文件下载，1:是，0:否|
|10|完整方法（类名+方法名+参数）|

## 1.46. Spring定时任务信息，通过注解定义
