...
public enum CallTypeEnum {
    ...
    CTE_RUNNABLE_INIT_RUN("_RIR", "Runnable 构造函数调用 run() 方法"),
    CTE_CALLABLE_INIT_CALL("_CIC", "Callable 构造函数调用 call() 方法"),
    CTE_TX_CALLBACK_INIT_CALL("_TCID", "TransactionCallback 构造函数调用 doInTransaction() 方法"),
    CTE_TX_CALLBACK_WR_INIT_CALL("_TCWRID", "TransactionCallbackWithoutResult 构造函数调用 doInTransactionWithoutResult() 方法"),
    ...
    public static void checkRepeat() {
        ...
        for (CallTypeEnum callTypeEnum : CallTypeEnum.values()) {
            ...
            if (enumNameList.size() > 1) {
                ...
                throw new JavaCGRuntimeException(CallTypeEnum.class.getSimpleName() + " 类定义的枚举类型存在重复" + callTypeEnum.getType() + " " + repeatedEnumTypes);
            }
        }
    }
